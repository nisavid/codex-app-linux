...
function applyMainBundlePatch(source) {
  ...
  const helper = [
    ...
    `function codexLinuxWrapSpawnCheck(){if(!codexLinuxWrapWrapperUpdatesEnabled()||!codexLinuxWrapManagerAvailable())return;try{let c=${childProcessVar}.spawn(codexLinuxWrapManagerPath(),[\`check-wrapper\`],{stdio:\`ignore\`,detached:!0,env:process.env});c.on(\`error\`,()=>{});c.unref()}catch{}}`,
