  ...
  withTempIntegrationConfig([], (root) => {
    ...
    assert.equal(
      loadPortIntegrationPatchDescriptors({ integrationsRoot: root })
        .some((patch) => patch.name.startsWith("integration:remote-control-ui:")),
      true,
    );
  });
});
  ...
  withTempIntegrationConfig(["remote-control-ui"], (root) => {
    ...
    assert.deepEqual(
      ...
      [
        ...
        ["integration:remote-control-ui:nux-gate", "webview-asset", "optional"],
        ["integration:remote-control-ui:app-main", "webview-asset", "optional"],
      ],
    );
  });
});
...
test("remote-control UI integration patches are idempotent and fail soft", () => {
  ...
  const appMainPatch = integrationPatches.find((patch) => patch.id === "app-main");
  const nuxGatePatch = integrationPatches.find((patch) => patch.id === "nux-gate");
      ...
      try {
        ...
        assert.ok(
          report.patches.some((patch) =>
            patch.name === "integration:remote-control-ui:remote-connections-visibility" && patch.status === "applied"
          ),
        );
      } finally {
