...
Codex App can launch successfully while shell commands inside the app still
...
Then ask Codex App to run the same command in a shell. If the terminal shows
...
# Electron/app-scoped XDG paths inherited by Codex App.
...
Then retry `~/.local/bin/gh-normal auth status` inside Codex App.
