...
const safeWebviewJavaScriptAssetName = /^[A-Za-z0-9][A-Za-z0-9._-]*\.js$/;

function jsStringLiteral(value) {
  return JSON.stringify(String(value)).replace(
    unsafeJavaScriptLiteralChars,
    (char) => unsafeJavaScriptLiteralEscapes[char],
  );
}

function assertSafeWebviewJavaScriptAssetName(value, description) {
  if (typeof value !== "string" || !safeWebviewJavaScriptAssetName.test(value)) {
    throw new Error(`Required Keybinds settings patch failed: unsafe ${description} ${JSON.stringify(value)}`);
  }
  return value;
}
...
function buildKeybindsSettingsSource({
  ...
  const imports = {
    chunk: jsModuleSpecifier(chunkAsset, "React shared chunk asset"),
    react: jsModuleSpecifier(reactAsset, "React asset"),
    jsxRuntime: jsModuleSpecifier(jsxRuntimeAsset, "JSX runtime asset"),
    vscodeApi: jsModuleSpecifier(vscodeApiAsset, "VS Code API asset"),
    hotkeySettings: jsModuleSpecifier(hotkeySettingsAsset, "hotkey settings asset"),
    toggle: jsModuleSpecifier(toggleAsset, "toggle asset"),
    settingsRow: jsModuleSpecifier(settingsRowAsset, "settings row asset"),
    settingsLayout: jsModuleSpecifier(settingsLayoutAsset, "settings content layout asset"),
  };
  const keybindGroups = [
    {
      title: "Core",
...
function resolveKeybindsSettingsAsset(extractedDir) {
  ...
  return {
    ...
    source: buildKeybindsSettingsSource({
      chunkAsset: assertSafeWebviewJavaScriptAssetName(chunkAsset, "React shared chunk asset"),
      reactAsset: assertSafeWebviewJavaScriptAssetName(reactAsset, "React asset"),
      jsxRuntimeAsset: assertSafeWebviewJavaScriptAssetName(jsxRuntimeAsset, "JSX runtime asset"),
      vscodeApiAsset: assertSafeWebviewJavaScriptAssetName(vscodeApiAsset, "VS Code API asset"),
      hotkeySettingsAsset: assertSafeWebviewJavaScriptAssetName(hotkeySettingsAsset, "hotkey settings asset"),
      toggleAsset: assertSafeWebviewJavaScriptAssetName(toggleAsset, "toggle asset"),
      settingsRowAsset: assertSafeWebviewJavaScriptAssetName(settingsRowAsset, "settings row asset"),
      settingsLayoutAsset: assertSafeWebviewJavaScriptAssetName(settingsLayoutAsset, "settings content layout asset"),
    }),
  };
}
...
function applyLinuxTrayPatch(currentSource, iconPathExpression) {
  ...
  if (patchedSource.includes(trayStartupPatch)) {
    ...
    if (
      traySetup != null &&
      patchedSource.includes(`${linuxTrayEnabledPredicate})&&${traySetup.setupFn}();`)
    ) {
      ...
      patchedSource = `${patchedSource.slice(0, dynamicTrayStartupMatch.index)}(${isWindowsVar}||${linuxTrayEnabledPredicate})&&${traySetup.setupFn}();${patchedSource.slice(dynamicTrayStartupMatch.index + dynamicTrayStartupMatch[0].length)}`;
    } else {
...
function applyLinuxSingleInstancePatch(currentSource) {
  ...
  if (
    ...
    patchedSource.includes("l(e=>{(typeof codexLinuxIsQuitInProgress===`function`&&codexLinuxIsQuitInProgress())?void 0:R.deepLinks.queueProcessArgs(e)||ie()})")
  ) {
    // Already patched.
...
function applyLinuxComputerUsePluginGatePatch(currentSource) {
  ...
  if (hasComputerUseLiteral(currentSource) && currentSource.includes("computerUse")) {
    console.warn(
      "WARN: Required Linux Computer Use plugin gate patch failed: skipping bundled Computer Use Linux enablement",
    );
  }
...
function buildSemanticLinuxLaunchActionPatch({
  ...
  const beforeQuitAppVar = appVar ?? electronVar;
  const beforeQuitHandler = beforeQuitAppVar == null
    ? ""
  ...
  const directHandler = appVar == null
    ? beforeQuitHandler
    : `,codexLinuxSecondInstanceHandler=(e,t)=>{codexLinuxHandleLaunchActionArgsFallback(t,()=>{${fallbackFn}()})}${beforeQuitHandler}`;
  const beforeQuitStartup = beforeQuitAppVar == null
    ? "codexLinuxStartLaunchActionSocket()"
    : `${beforeQuitAppVar}.app.on(\`before-quit\`,codexLinuxBeforeQuitHandler),${disposableVar}.add(()=>{${beforeQuitAppVar}.app.off(\`before-quit\`,codexLinuxBeforeQuitHandler)}),codexLinuxStartLaunchActionSocket()`;
  const startup = appVar == null
    ? `process.platform===\`linux\`&&(${beforeQuitStartup});${setterVar}(e=>{codexLinuxHandleLaunchActionArgsFallback(e,()=>{${fallbackFn}()})});`
...
function applyLinuxLaunchActionArgsPatch(currentSource) {
  ...
  const launchActionNeedleWithQuitGuard =
    "let codexLinuxSecondInstanceHandler=(e,t)=>{(typeof codexLinuxIsQuitInProgress===`function`&&codexLinuxIsQuitInProgress())?void 0:R.deepLinks.queueProcessArgs(t)||ie()},codexLinuxBeforeQuitHandler=()=>{typeof codexLinuxMarkQuitInProgress===`function`&&codexLinuxMarkQuitInProgress()};process.platform===`linux`&&(n.app.on(`before-quit`,codexLinuxBeforeQuitHandler),k.add(()=>{n.app.off(`before-quit`,codexLinuxBeforeQuitHandler)}),n.app.on(`second-instance`,codexLinuxSecondInstanceHandler),k.add(()=>{n.app.off(`second-instance`,codexLinuxSecondInstanceHandler)})),l(e=>{(typeof codexLinuxIsQuitInProgress===`function`&&codexLinuxIsQuitInProgress())?void 0:R.deepLinks.queueProcessArgs(e)||ie()});let ae=async(e,t)=>{P.hotkeyWindowLifecycleManager.hide();let n=P.getPrimaryWindow(z),r=n??await P.createFreshLocalWindow(e);r!=null&&(n!=null&&t.navigateExistingWindow&&R.navigateToRoute(r,e),re(r))},oe=async()=>{";
  ...
  if (
    ...
    patchedSource.includes("codexLinuxHandleLaunchActionArgs=async e=>") &&
  ...
  if (patchedSource.includes(oldLaunchActionPatch)) {
  ...
  } else if (patchedSource.includes(launchActionNeedleWithQuitGuard)) {
    patchedSource = patchedSource.replace(launchActionNeedleWithQuitGuard, launchActionPatch);
  } else {
    ...
    if (semanticLaunchActionPatch !== patchedSource) {
      return applyLinuxQuitGuardPatch(semanticLaunchActionPatch);
    }

    if (patchedSource.includes(currentLaunchActionNeedle)) {
      patchedSource = patchedSource.replace(currentLaunchActionNeedle, currentLaunchActionPatch);
      return applyLinuxQuitGuardPatch(patchedSource);
    }
...
function applyLinuxHotkeyWindowPrewarmPatch(currentSource) {
  ...
  const currentStartupPrewarmNeedle =
  ...
  } else if (patchedSource.includes(currentStartupPrewarmNeedle)) {
    patchedSource = patchedSource.replace(
      currentStartupPrewarmNeedle,
      "w(`local window ensured`,A,{hostId:B,localWindowVisible:be?.isVisible()??!1}),process.platform===`linux`&&codexLinuxPrewarmHotkeyWindow(),A=Date.now(),await z.deepLinks.flushPendingDeepLinks()",
    );
  } else if (
