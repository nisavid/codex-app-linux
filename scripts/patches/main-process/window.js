...
function applyLinuxOpaqueBackgroundPatch(currentSource) {
  ...
  const legacyUrlPredicateLinuxBgRegex = new RegExp(
    `backgroundMaterial:\`mica\`}:process\\.platform===\`linux\`&&![A-Za-z_$][\\w$]*\\(${escapeRegExp(appearanceParam)}\\)\\?\\{backgroundColor:${escapeRegExp(darkColorsParam)}\\?${escapeRegExp(darkVar)}:${escapeRegExp(lightVar)},backgroundMaterial:null\\}:\\{backgroundColor:${escapeRegExp(transparentVar)},backgroundMaterial:null\\}\\}`,
  );

  if (patchedSource.includes(bgNeedle)) {
    return patchedSource.replace(bgNeedle, bgReplacement);
  }
  if (patchedSource.includes(bgReplacement)) {
    return patchedSource;
  }
  if (patchedSource.includes(oldLinuxBgPatch)) {
    return patchedSource.replace(oldLinuxBgPatch, bgReplacement);
  }
  if (legacyUrlPredicateLinuxBgRegex.test(patchedSource)) {
    return patchedSource.replace(legacyUrlPredicateLinuxBgRegex, bgReplacement);
  }
