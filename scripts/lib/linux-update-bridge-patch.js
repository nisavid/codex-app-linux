...
function migrateLinuxUpdaterBridgeSource(source) {
  ...
  const bootstrapNeedle = "function codexLinuxCreatePackageAppUpdater(";
  ...
  patchedSource = patchedSource.replace(
    ...
    "await codexLinuxProbeAppUpdater(),e()",
  );

  const probeStateSource =
    "let s=!1,c=codexLinuxProbeAppUpdater().then(()=>{s=!0,i(),a();return!0}).catch(()=>{s=!1,t=!1,n=`idle`,a();return!1});let o=";
  const hasProbeState = () => patchedSource.includes("c=codexLinuxProbeAppUpdater().then(");
    ...
    if (legacyBridgeRegex.test(patchedSource)) {
      patchedSource = patchedSource.replace(
        ...
        (_match, quitControllerVar, quitFactoryVar, quitFnVar, electronBindingVar) =>
          `let ${quitControllerVar}=${quitFactoryVar}(),${quitFnVar}=()=>{${quitControllerVar}.allowQuitTemporarilyForUpdateInstall(),${electronBindingVar}.app.quit()},codexLinuxPackageUpdateBridge=process.platform===\`linux\`?codexLinuxCreatePackageAppUpdater({allowQuit:()=>${quitControllerVar}.allowQuitTemporarilyForUpdateInstall(),${sendCallback}}):null;codexLinuxPackageUpdateBridge!=null&&(${sparkleVar}=codexLinuxPackageUpdateBridge.manager,${quitFnVar}=codexLinuxPackageUpdateBridge.quitForUpdate,setInterval(()=>codexLinuxPackageUpdateBridge.refresh(),3e4).unref?.());`,
      );
    } else {
...
function applyLinuxAppUpdaterBridgePatch(currentSource) {
  ...
  if (!patchedSource.includes("async initializeLinuxPackageUpdater(){")) {
    ...
    const methodPatch =
      "async initializeLinuxPackageUpdater(){if(process.platform!==`linux`){this.lastUnavailableReason=`unsupported platform`;return}let e=()=>{let e=codexLinuxReadUpdateState(),t=e?.status;this.setUpdateReady(t===`ready_to_install`||t===`waiting_for_app_exit`),this.setUpdateLifecycleState(codexLinuxUpdateLifecycleState(t)),this.lastUnavailableReason=null;return e};try{await codexLinuxProbeAppUpdater(),e()}catch(e){this.lastUnavailableReason=e?.code===`ENOENT`?`codex-app-updater not found`:`codex-app-updater unavailable`,ZE().warning(`Linux updater unavailable`,{safe:{reason:this.lastUnavailableReason},sensitive:{error:e}});return}this.updater={setAutomaticBackgroundDownloadsEnabled:()=>{},checkForUpdates:async()=>{this.setUpdateLifecycleState(`checking`);try{await codexLinuxRunAppUpdater([`check-now`]),e()}catch(t){this.setUpdateLifecycleState(this.isUpdateReady?`ready`:`idle`);throw t}},installUpdatesIfAvailable:async()=>{e();if(!this.isUpdateReady)return;this.setInstallProgressPercent(0),this.setUpdateLifecycleState(`installing`);try{let n=await codexLinuxRunAppUpdater([`install-ready`]),t=e();if(t?.status===`waiting_for_app_exit`){this.setInstallProgressPercent(null),codexLinuxQuitForUpdate();return}this.setInstallProgressPercent(null),n.stdout?.includes(`Manual install required:`)?await codexLinuxShowUpdateMessage(`Codex App update`,n.stdout.trim()):n.stdout?.includes(`already installed`)?await codexLinuxShowUpdateMessage(`Codex App update`,`The ready update is already installed.`):n.stdout?.includes(`No Codex App update is ready`)&&await codexLinuxShowUpdateMessage(`Codex App update`,`There is no rebuilt update waiting to install.`)}catch(e){this.setInstallProgressPercent(null),this.setUpdateLifecycleState(this.isUpdateReady?`ready`:`idle`);throw e}}};let t=setInterval(()=>{codexLinuxRefreshUpdateState().then(()=>e()).catch(e=>{ZE().warning(`Linux updater state refresh failed`,{safe:{},sensitive:{error:e}})})},3e4);t.unref?.()}";
