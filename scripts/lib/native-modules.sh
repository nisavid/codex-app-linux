...
build_native_modules() {
    ...
    npm install \
        ...
        --save-dev \
        --ignore-scripts >&2 2>&1
    npm install "better-sqlite3@$bs3_build_ver" "node-pty@$npty_ver" --ignore-scripts >&2 2>&1
    ...
    NPM_CONFIG_DISTURL="$ELECTRON_HEADERS_URL" \
    node "$build_dir/node_modules/@electron/rebuild/lib/cli.js" -v "$ELECTRON_VERSION" --force --dist-url "$ELECTRON_HEADERS_URL" >&2 2>&1
